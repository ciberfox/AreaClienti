...
class PHPLogin
{
    ...
    private function ExecuteAction()
    {
        ...
        if (isset($_POST["captcha"]) && isset($_POST["register"]) && (ALLOW_USER_REGISTRATION || (ALLOW_ADMIN_TO_REGISTER_NEW_USER && $_SESSION['user_access_level'] == 255))) {
            $this->registerNewUser($_POST['user_name'], $_POST['user_email'], $_POST['codice_contabile'], $_POST['user_password_new'], $_POST['user_password_repeat'], $_POST["captcha"]);
     ...
     * everything is fine
     */
    private function registerNewUser($user_name, $user_email, $codice_contabile, $user_password, $user_password_repeat, $captcha)
    {
        // prevent database flooding
        $user_name = substr(trim($user_name), 0, 64);
        $user_email = substr(trim($user_email), 0, 254);
        ...
        /*else if (strtoupper($captcha) != strtoupper($_SESSION['captcha'])) {
        ...
        // finally if all the above checks are ok
            ...
            // Ok user can be create
                ...
                $query_new_user_insert = $this->db_connection->prepare('INSERT INTO users (user_name, user_password_hash, user_email, codice_contabile, user_activation_hash, user_registration_ip, user_registration_datetime) VALUES(:user_name, :user_password_hash, :user_email, :codice_contabile, :user_activation_hash, :user_registration_ip, now())');
                $query_new_user_insert->bindValue(':user_name', $user_name, PDO::PARAM_STR);
                $query_new_user_insert->bindValue(':user_password_hash', $user_password_hash, PDO::PARAM_STR);
                $query_new_user_insert->bindValue(':user_email', $user_email, PDO::PARAM_STR);
                $query_new_user_insert->bindValue(':codice_contabile', $codice_contabile, PDO::PARAM_STR);            
